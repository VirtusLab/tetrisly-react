...
describe('check tetrisly provider, and correct styling', () => {
  it('with no theme provided should use default theme', () => {
    render(
      <TetrislyProvider>
        <tet.button color="$color-blue-+1">Click me</tet.button>
      </TetrislyProvider>,
    );
    const button = screen.getByText('Click me');
    expect(button).toHaveStyle(`color: ${theme.colors['$color-blue-+1']}`);
  });
  it('with theme provided should use theme', () => {
    ...
    render(
      <TetrislyProvider
        theme={{
          colors: {
            '$color-blue-+1': customColor,
          },
        }}
      >
        <tet.button color="$color-blue-+1" backgroundColor="$color-blue-+2">
          Click me
        </tet.button>
      </TetrislyProvider>,
    );
  ...
  it('with theme provided should replace default theme when same key is provided', () => {
    ...
    expect(button).toHaveStyle(
      `background-color: ${theme.colors['$color-blue-+2']}`,
    );
  });
});
